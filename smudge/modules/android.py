...
async def check(c: Client, update: Update):
    ...
    if page1.find("latest").text.strip():
        ...
        message = tld(chat_id, "samsung_releases_details").format(
    ...
    if cmd in ("checkfw", "check"):
        ...
        for site_name, fw_link in fw_links:
            keyboard += [[InlineKeyboardButton(site_name,
                                               url=fw_link.format(model.upper(), csc.upper()))]]
        await c.send_message(
            chat_id=update.chat.id,
            text=message,
            reply_markup=InlineKeyboardMarkup(keyboard))
...
async def models(c: Client, update: Update):
    ...
    if data:
        ...
        await c.send_message(
            ...
            text=message)
        return
    message = tld(chat_id, "device_result").format(
        brand, name, device, model.upper())
    await c.send_message(
        chat_id=update.chat.id,
        text=message,
        disable_web_page_preview=True
