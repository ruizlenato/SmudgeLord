...
from smudge import dispatcher, LOGGER, SUDO_USERS
from smudge.modules.connection import connected
from smudge.modules.disable import DisableAbleCommandHandler
from smudge.modules.helper_funcs.chat_status import bot_admin, user_admin, is_user_admin, can_restrict
...
def mute(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if not user_id:
        message.reply_text(
            tld(chat.id, "You'll need to either give me a username to mute, or reply to someone to be muted."))
        return ""
    ...
    if member:
        ...
        if user_id in SUDO_USERS:
            ...
            keyboard = []
            reply = tld(chat.id, "{} is muted in {}!").format(mention_html(member.user.id, member.user.first_name),
                                                              chatD.title)
...
def unmute(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if not user_id:
        message.reply_text(
            tld(chat.id, "You'll need to either give me a username to unmute, or reply to someone to be unmuted."))
        return ""
    ...
    if member.status != 'kicked' and member.status != 'left':
        if member.can_send_messages and member.can_send_media_messages \
            ...
            keyboard = []
            reply = tld(chat.id, "Yep, {} can start talking again in {}!").format(
        ...
        message.reply_text(
            tld(chat.id, "This user isn't even in the chat, unmuting them won't make them talk more than they "
                         "already do!"))
    ...
    try:
        if member.can_send_messages is None or member.can_send_messages:
            bot.restrict_chat_member(chat.id, user_id, until_date=mutetime, can_send_messages=True,
                                     ...
                                     can_add_web_page_previews=False)
            message.reply_text(
                tld(chat.id, "Restricted from sending media for {} in {}!").format(time_val, chatD.title))
            return log
        else:
...
TEMP_NOMEDIA_HANDLER = DisableAbleCommandHandler(["trestrict", "temprestrict"], temp_nomedia, pass_args=True,
                                                 admin_ok=True)
